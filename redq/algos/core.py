...
# TH 202202145
from .detachLNTH import LayerNormalizationWOVarianceReScale

# TH 20221014
from tod.tod import TurnOverDropout
...
class ReplayBuffer:
    ...
    def __init__(self, obs_dim, act_dim, size):
        ...
        # unique id for each transition. TH20221014
        self.ids_buf = np.zeros(size, dtype=np.int32)
        self.unique_id = 0
    ...
    def store(self, obs, act, rew, next_obs, done):
        ...
        # TH 20221014
        self.ids_buf[self.ptr] = self.unique_id
    ...
    def sample_batch(self, batch_size=32, idxs=None):
        ...
        return dict(obs1=self.obs1_buf[idxs],
                    ...
                    ids=self.ids_buf[idxs], # TH20221014
                    idxs=idxs)
...
class Mlp(nn.Module):
    def __init__(
        ...
        for i, next_size in enumerate(hidden_sizes):
            ...
            # ToD, DO, LN. TH 20221014
            if self.turn_over_dropout_rate > 0.0:
                if ("ToD" in apply_only_last_layer) and (i != ( len(hidden_sizes) - 1 ) ):
                    self.hidden_layers.append(nn.Identity())
                else:
                    self.hidden_layers.append(TurnOverDropout(size=in_size, p=self.turn_over_dropout_rate))

            # added 20211206
            if target_drop_rate > 0.0:
                if ("DO" in apply_only_last_layer) and (i != ( len(hidden_sizes) - 1 ) ):
                    self.hidden_layers.append(nn.Identity())
                else:
                    self.hidden_layers.append(nn.Dropout(p=target_drop_rate))  # dropout

            if layer_norm:
                if ("LN" in apply_only_last_layer) and (i != ( len(hidden_sizes) - 1 ) ):
                    self.hidden_layers.append(nn.Identity())
                else:
                    self.hidden_layers.append(nn.LayerNorm(fc_layer.out_features))  # layer norm

        # added to fix bug. TH 20211207
    ...
    def forward(self, input,
                indices = None, flips = None # for ToD. TH 20221015
                ):
        h = input
        ...
        for i, fc_layer in enumerate(self.hidden_layers):
            ...
            # foward ToD. TH 20221015
            if self.turn_over_dropout_rate > 0.0:
                if type(fc_layer) is TurnOverDropout:
                    h = fc_layer(x=h, indices=indices, flips=flips)
                else:
                    h = fc_layer(h)
            else:
                h = fc_layer(h)
...
class TanhGaussianPolicy(Mlp):
    ...
    def __init__(
            ...
            hidden_activation=F.relu,
            action_limit=1.0,
            turn_over_dropout_rate = 0.0, # TH 20221014
            apply_only_last_layer = [], # TH 20221024
            layer_norm = False
    ):
        super().__init__(
            input_size=obs_dim,
            ...
            turn_over_dropout_rate = turn_over_dropout_rate, # TH20221014
            apply_only_last_layer = apply_only_last_layer,
        ...
        self.turn_over_dropout_rate = turn_over_dropout_rate # TH20221015
