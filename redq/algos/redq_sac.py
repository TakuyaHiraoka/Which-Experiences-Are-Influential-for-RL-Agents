...
class REDQSACAgent(object):
    ...
    def __init__(self, env_name, obs_dim, act_dim, act_limit, device,
        ...
        # for this setting. layer norm and tod is not applied 20230512
        # set up networks
        self.policy_net = TanhGaussianPolicy(obs_dim, act_dim, hidden_sizes,
        ...
        if auto_alpha:
            ...
            if target_entropy == 'mbpo':
                # set additional target entropy for additional environments. TH 20211206
        ...
        self.replay_buffer = ReplayBuffer(obs_dim=obs_dim,
                                          act_dim=act_dim,
                                          size=replay_size,
                                          hidden_sizes=hidden_sizes, # Add hidden size for mask creation. TH 20230410
                                          )

        # for validation # TODO prepare only when specified? This makes agent more memory intensive #TH20230419
        self.replay_buffer_validation = ReplayBuffer(obs_dim=obs_dim,
                                          act_dim=act_dim,
                                          size=replay_size,
                                          hidden_sizes=hidden_sizes,  # Add hidden size for mask creation. TH 20230410
                                          )
    ...
    def get_test_action(self, obs, masks=None, flips=False): # extend to masking. TH20240324
        # given an observation, output a deterministic action in numpy form
        with torch.no_grad():
            obs_tensor = torch.Tensor(obs).unsqueeze(0).to(self.device)
            action_tensor = self.policy_net.forward(obs_tensor, deterministic=True, return_log_prob=False,
                                                    masks=masks, flips=flips)[0]
            action = action_tensor.cpu().numpy().reshape(-1)
        return action
    ...
    def get_ave_q_prediction_for_bias_evaluation(self, obs_tensor, acts_tensor,
                                                 masks=None, flips=None  # for ToD. TH 20221026
                                                 ):
        ...
        for q_i in range(self.num_Q):
            q_prediction = self.q_net_list[q_i](torch.cat([obs_tensor, acts_tensor], 1),
                                                masks=masks, flips=flips) # TH 20221026
    ...
    # TH20230419
    def store_data_validation(self, o, a, r, o2, d):
        self.replay_buffer_validation.store(o, a, r, o2, d)
    ...
    def sample_data(self, batch_size):
        ...
        masks_tensor = Tensor(batch['masks']).to(self.device) #TH20230410
    ...
    def get_redq_q_target_no_grad(self, obs_next_tensor, rews_tensor, done_tensor,
                                  masks_tensor=None, flips=False): # TH20221015
        ...
        with torch.no_grad():
            if self.q_target_mode == 'min':
                ...
                a_tilda_next, _, _, log_prob_a_tilda_next, _, _ = self.policy_net.forward(obs_next_tensor,
                                                                                          masks=masks_tensor, # use Tod if flag is activated. TH20221015
                                                                                          flips=flips
                                                                                          )
                ...
                for sample_idx in sample_idxs:
                    q_prediction_next = self.q_target_net_list[sample_idx](torch.cat([obs_next_tensor, a_tilda_next], 1),
                                                                           masks=masks_tensor,
                                                                           flips=flips # use Tod if flag is activated. TH20221015
                                                                           )
            ...
            if self.q_target_mode == 'ave':
                assert False, "ToD version currently does not support average operator. "
            ...
            if self.q_target_mode == 'rem':
                assert False, "ToD version currently does not support REM operator. "
    ...
    def train(self, logger):
        ...
        for i_update in range(num_update):
            # + ids_tensor. TH20221015
            obs_tensor, obs_next_tensor, acts_tensor, rews_tensor, done_tensor, masks_tensor = self.sample_data(self.batch_size)

            """Q loss"""
            # + ids_tensor and flip flag. TH20221015
            y_q, sample_idxs = self.get_redq_q_target_no_grad(obs_next_tensor, rews_tensor, done_tensor,
                                                              masks_tensor = masks_tensor, flips=False) # use ToD if flag is activate. TH20221015
            q_prediction_list = []
            for q_i in range(self.num_Q):
                q_prediction = self.q_net_list[q_i](torch.cat([obs_tensor, acts_tensor], 1),
                                                    masks=masks_tensor,
            ...
            if i_update == num_update - 1:
                # LossQ1Grad
                with torch.no_grad():
                    lgrad = 0 #torch.mean(self.q_net_list[0]._modules['hidden_layers']._modules['0'].weight.grad).cpu().data.item() # TODO ignore in ToEnsemble TH20240318
                ...
                logger.store(LossPi=policy_loss.cpu().item(), LossQ1=q_loss_all.cpu().item() / self.num_Q,
                             ...
                             LossQ1Grad=lgrad) # TH20220208


        # if there is no update, log 0 to prevent logging problems. TH 20220208
        if num_update == 0:
            logger.store(LossPi=0, LossQ1=0, LossAlpha=0, Q1Vals=0, Alpha=0, LogPi=0, PreTanh=0, LossQ1Grad=0)
