...
class REDQSACAgent(object):
    ...
    def __init__(self, env_name, obs_dim, act_dim, act_limit, device,
                 ...
                 turn_over_dropout_rate=0.0, # TH20221014
                 apply_only_last_layer_Q=[], # TH 20221107
                 apply_only_last_layer_Policy=[], # TH 20221107
                 layer_norm_policy = False # TH 20221024
                 ):

        # TH20221014
        self.turn_over_dropout_rate = turn_over_dropout_rate

        # set up networks
        self.policy_net = TanhGaussianPolicy(obs_dim, act_dim, hidden_sizes,
                                             action_limit=act_limit,
                                             turn_over_dropout_rate = self.turn_over_dropout_rate, # TH 20221014
                                             apply_only_last_layer=apply_only_last_layer_Policy,  # TH 20221024 <-- updated 20221107
        ...
        for q_i in range(num_Q):
            new_q_net = Mlp(obs_dim + act_dim, 1, hidden_sizes,
                            ...
                            turn_over_dropout_rate = self.turn_over_dropout_rate, # TH 20221014
                            apply_only_last_layer = apply_only_last_layer_Q # TH 20221024 <-- updated 20221107
                            ).to(device)
    ...
    def get_ave_q_prediction_for_bias_evaluation(self, obs_tensor, acts_tensor,
                                                 indices=None, flips=None  # for ToD. TH 20221026
                                                 ):
        ...
        for q_i in range(self.num_Q):
            q_prediction = self.q_net_list[q_i](torch.cat([obs_tensor, acts_tensor], 1),
                                                indices=indices, flips=flips) # TH 20221026
    ...
    def sample_data(self, batch_size):
        ...
        ids_tensor = Tensor(batch['ids']).unsqueeze(1).to(self.device)
        return obs_tensor, obs_next_tensor, acts_tensor, rews_tensor, done_tensor, ids_tensor

    def get_redq_q_target_no_grad(self, obs_next_tensor, rews_tensor, done_tensor,
                                  ids_tensor=None, flips=None): # TH20221015
        ...
        with torch.no_grad():
            if self.q_target_mode == 'min':
                ...
                a_tilda_next, _, _, log_prob_a_tilda_next, _, _ = self.policy_net.forward(obs_next_tensor,
                                                                                          indices=ids_tensor, # use Tod if ids are not None. TH20221015
                                                                                          flips=flips
                                                                                          )
                q_prediction_next_list = []
                for sample_idx in sample_idxs:
                    q_prediction_next = self.q_target_net_list[sample_idx](torch.cat([obs_next_tensor, a_tilda_next], 1),
                                                                           indices=ids_tensor, flips=flips # use Tod if ids are not None. TH20221015
                                                                           )
    ...
    def train(self, logger):
        ...
        for i_update in range(num_update):
            ...
            obs_tensor, obs_next_tensor, acts_tensor, rews_tensor, done_tensor, ids_tensor = self.sample_data(self.batch_size)
            ...
            y_q, sample_idxs = self.get_redq_q_target_no_grad(obs_next_tensor, rews_tensor, done_tensor,
                                                              ids_tensor= ids_tensor, flips=False)
            q_prediction_list = []
            for q_i in range(self.num_Q):
                q_prediction = self.q_net_list[q_i](torch.cat([obs_tensor, acts_tensor], 1),
                                                    indices=ids_tensor,
            ...
            if i_update == num_update - 1:
                # LossQ1Grad
                with torch.no_grad():
                    lgrad = torch.mean(self.q_net_list[0]._modules['hidden_layers']._modules['0'].weight.grad).cpu().data.item()
